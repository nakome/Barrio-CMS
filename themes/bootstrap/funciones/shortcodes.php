...
Barrio::shortcodeAdd('Bloque', function ($atributos,$contenido) {
    ...
    // enseñar
    $contenido = Barrio::applyFilter('content', '<div class="col-md-'.$col.'">'.$contenido.'</div>');
...
Barrio::shortcodeAdd('Bloque_izq', function ($atributos,$contenido) {
    ...
    if ($img) {
        ...
        $html .='</div>';
        $html .= Barrio::applyFilter('content', '<div class="col-md'.($col-12).'">'.$contenido.'</div>');
        $html .='</div>';
        $html = preg_replace('/\s+/', ' ', $html);
        return $html;
    } else {
...
Barrio::shortCodeAdd('Card',function($atributos,$contenido){
    ...
    $html .= '    <h4 class="card-title m-1 text-success">'.$title.'</h4>';
    $html .=      $resultado;
    $html .= '  </div>';
    $html .= '</div>';
    $html .= '</div>';
